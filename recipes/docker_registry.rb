...
bash "start_docker_registry" do
  user "root"
  code <<-EOF
   docker run -d --restart=always --name registry -v #{kagent_crypto_dir}:/certs -e REGISTRY_HTTP_ADDR=0.0.0.0:443 -e REGISTRY_HTTP_TLS_CERTIFICATE=/certs/#{certificate_name} -e REGISTRY_HTTP_TLS_KEY=/certs/#{key_name} -e REGISTRY_STORAGE_DELETE_ENABLED=true -p #{node['hops']['docker']['registry']['port']}:443 registry
   EOF
